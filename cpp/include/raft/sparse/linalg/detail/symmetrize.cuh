                    ...
                    cudaStream_t stream)
{
  ...
  raft::launch_kernel(stream, grid, blk)(coo_symmetrize_kernel<TPB_X, T>,
                                         in_row_ind.data(),
                                         in->rows(),
                                         in->cols(),
                                         in->vals(),
                                ...
                                cudaStream_t stream)
{
  ...
  raft::launch_kernel(stream, numBlocks, threadsPerBlock)(
    symmetric_find_size, knn_dists, knn_indices, n, k, row_sizes.data(), row_sizes2.data());

  raft::launch_kernel(stream, raft::ceildiv(n, (value_idx)1024), 1024)(
    reduce_find_size, n, k, row_sizes.data(), row_sizes2.data());
  ...
  raft::launch_kernel(stream, numBlocks, threadsPerBlock)(
    symmetric_sum, edges, knn_dists, knn_indices, out->vals(), out->cols(), out->rows(), n, k);
}
